...
from helper_functions.Gemini_handler import transcribe_audio_with_gemini # Explicitly import
...
async def initialize_chat(background_tasks: BackgroundTasks, payload: dict = Body(...)):
    ...
    def process_initial_data(email: str, voice_data: str, patient_data: dict):
        ...
        if history_response:
            print(f"Conversation history inserted successfully: {history_response.data}")
        else:
        ...
        patient_insert_response = supabase_handler.insert_patient_info(patient_data_to_insert)
    ...
    background_tasks.add_task(process_initial_data, payload.get("email", ""), voice, data)
...
async def generate_diagnosis(background_tasks: BackgroundTasks, payload: list = Body(...)):
    ...
    email = ""
    if payload and isinstance(payload, list) and len(payload) > 0 and "email" in payload[0]:
    ...
    def process_diagnosis_data(email: str, qa_pairs: list):
        ...
        if email:
            ...
            history_response = supabase_handler.conversation_history(conversation_entry)
            if history_response:
                print(f"Follow-up conversation history inserted successfully: {history_response.data}")
            else:
        ...
        diagnosis_prompt = DIFFERENTIAL_DIAGONOSIS_GENERATION_PROMPT.replace("[[patient_details]]", full_conversation_text)
        diagnosis_data = Process_voice_with_Gemini("", prompt=diagnosis_prompt) # No voice input here, just text prompt
    ...
    return {"status": "success", "message": "Diagnosis processing initiated in background."}
